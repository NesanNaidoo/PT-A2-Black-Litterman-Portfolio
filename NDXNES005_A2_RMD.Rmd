...
# Mean and Variance of portfolio In-Sample vs Out-of-Sample 
p1<- p.long |>
...
# Sharpe Ratios of portfolios In-Sample vs Out-of-Sample 
p2 <- p.long |>
  filter(Metric == "SR") |>
...
## Validation checks
```{r}
head(rets_opt)    # returns matrix for equities (JSE indices)
head(rets_cash)   # returns matrix for cash (STEFI)
...
# Check cash
if(!is.null(rets_cash)) summary(rets_cash)
...
## Test Case
```{r}
Pi_test <- c(0.25,0.1,0.05)
Sigma_test <- matrix(c(0.09,0.024,-0.006,
                       0.024,0.01,0.0003,
                      -0.006,0.0003,0.0025), 3,3, byrow=TRUE)
P_test <- matrix(c(1,-1,0,
                   0,1,-1),2,3, byrow=TRUE)
Q_test <- c(0.2,-0.05)
Omega_test <- matrix(c(0.3,0,0,0.55),2,2)

```

## No NA's
```{r}
rets_clean <- na.locf(rets_opt)  # removes rows with any NA
# Report dimensions
cat("Original rets_opt dimensions:", dim(rets_opt), "\n")
cat("Dimensions after removing NA rows:", dim(rets_clean), "\n")
