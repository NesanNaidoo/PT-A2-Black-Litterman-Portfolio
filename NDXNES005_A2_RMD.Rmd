...
results: asis
---
#  PART II : Implement and Historically Simulate the Black-Litterman Model in a R script le
...
```{r}
### Rolling-window Black-Litterman Backtest with Turnover, Beta, and Jensen Alpha

library(PerformanceAnalytics) # for CAPM functions

train.m <- 60  # 5-year training period
test.m  <- 12  # 1-year test period
roll_step <- 1 # 1-month increments
n_obs <- nrow(rets_opt)
start_idxs <- seq(1, n_obs - train.m - test.m + 1, by=roll_step)
results <- list()
prev_w <- rep(0, ncol(rets_opt)) # initialize previous weights for turnover

# ALSI column for beta calculation
...
for(i in seq_along(start_idxs)){
  ...
  P[1, 1:3] <- 1/3       # simple example view
  Q <- matrix(0.01, nrow=1) # expected 1% return
  Omega <- matrix(0.0001, nrow=1) # view uncertainty
  
  bl_res <- blacklittermann(Pi=mu_train, Sigma=Sigma_train, P=P, Q=Q, Omega=Omega, tau=0.05, gamma=3,constrain=FALSE)
  
  # --- Constrain weights: no short, fully invested ---
...
tracking_error <- sd(port_diff, na.rm = TRUE)
  
  # --- Jensen Alpha & Beta relative to ALSI ---
  # In-sample excess returns
excess_port_train  <- port_train  - rf_train
excess_bench_train <- as.numeric(bench_train) - rf_train

CAPM_train <- lm(excess_port_train ~ excess_bench_train)
alpha_IS <- coef(CAPM_train)[1]
beta_IS  <- coef(CAPM_train)[2]

# Out-of-sample excess returns
...
beta_OOS  <- coef(CAPM_test)[2]
  ...
  beta_OOS = beta_OOS,
  turnover = turnover,
  tracking_error = tracking_error,   # ✅ <-- added
...
knitr::kable(summary_df,
  digits = 4,
  caption = "In-sample vs Out-of-sample BL Portfolio Statistics with Turnover, Alpha and Beta"
)
...
### Jensens alpha and beta
```{r}
library(ggplot2)

# Create data frame with both in-sample and out-of-sample Alpha and Beta
...
ggplot(alpha_beta_df, aes(x = Window)) +
...
# Mean
ggplot(subset(summary_long, Metric == "Mean"),
...
# Variance 
ggplot(subset(summary_long, Metric == "Variance"),
       aes(x = Window, y = Value, color = Sample, linetype = Sample)) +
  geom_line(linewidth = 0.9) +
  labs(
    title = "",
...
# Sharpe Ratio 
ggplot(subset(summary_long, Metric == "Sharpe"),
       aes(x = Window, y = Value, color = Sample, linetype = Sample)) +
  geom_line(linewidth = 0.9) +
  labs(
    title = "",
...
```{r}
### Tracking Error Plot
tracking_error_df <- data.frame(
  Window = 1:length(results),
  TrackingError = sapply(results, function(x) x$tracking_error)
)
# Rolling Window Tracking Error vs ALSI
ggplot(tracking_error_df, aes(x = Window, y = TrackingError)) +
  geom_line(linewidth = 0.9) +
